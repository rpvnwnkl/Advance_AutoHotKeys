...
Reload
Return
fileName = deceased_Telefund.csv
startFile()
{
    global ;
    Loop, Read, %fileName%
    {
        ...
        IfNotInString, A_LoopReadLine, `n
        {
            ;an empty file
            MsgBox, Empty File
            textToLog = EntityId, Comment`n
            FileAppend, %textToLog%, %fileName%
        }
    }
}

writeToFile(entityID, Comment)
{
    global ;
...
, 
    IfWinNotActive, Production Database
, , WinActivate, Production Database
, 
...
IfEqual, callResult, %Deceased%
{
    ...
    startFile()
    writeToFile(callID, callComment)
}
